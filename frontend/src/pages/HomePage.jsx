...
useEffect(() => {
  const fetchNotebooks = async () => {
    try {
      ...
      const res = await axios.get(`${API}/api/notebooks`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
...
return (
  <div className="min-h-screen bg-black text-white flex">
    ...
    <aside className="w-[24%] min-w-[280px] bg-zinc-900 border-r border-zinc-800 p-6 flex flex-col gap-6">
      ...
      <button onClick={() => navigate("/dashboard")} className="mt-4 py-2 rounded-lg border border-zinc-700 hover:bg-zinc-800 transition cursor-pointer">
        Dashboard
      </button>
      <button
