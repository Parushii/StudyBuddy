...
const subject = notebook?.name || "Unknown";
    ...
    const trackVisit = async () => {
      if (!notebook) return;
      try {
        await fetch(`${API}/api/progress/topic-visit`, {
          method: "POST",
    ...
    const sendTimeSpent = async (seconds) => {
      try {
        await fetch(`${API}/api/progress/time`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            userId: localStorage.getItem("userId"),
    ...
    const handleSubmit = async () => {
      ...
      await sendTimeSpent(timeSpent);
      try {
        const finalScore = quizData.filter((q, i) => answers[i] === q.answer).length;
        await fetch(`${API}/api/progress/submit`, {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            userId: localStorage.getItem("userId"),
            notebookId,
        ...
        <div style={{ maxWidth: 720, margin: "0 auto", padding: "32px 24px" }}>
          ...
          <div style={{ display: "flex", gap: 10, marginBottom: 28, flexWrap: "wrap" }}>
            <button onClick={() => navigate("/notebookview/" + notebookId)}
              style={{
                padding: "8px 18px", borderRadius: 12, fontFamily: "Georgia, serif", fontSize: 14,
        ...
        <div style={{
          ...
          <div style={{ padding: "52px 40px", textAlign: "center" }}>
            ...
            <div style={{ marginTop: 20, display: "flex", gap: 6, justifyContent: "center" }}>
              {[0, 1, 2].map(i => (
                <div key={i} style={{
                  width: 8, height: 8, borderRadius: "50%", backgroundColor: "#c4956a",
    ...
    if (error) return (
      <PageWrap>
        <div style={{
          position: "relative", borderRadius: 20, border: "2.5px solid #c94040",
          ...
          <div style={{ padding: "48px 40px", textAlign: "center" }}>
            ...
            <button onClick={() => navigate("/notebookview/" + notebookId)}
              style={{
                padding: "10px 24px", borderRadius: 12, fontFamily: "Georgia, serif",
      ...
      <PageWrap>
        ...
        <div style={{
          ...
          <div style={{ padding: "36px 40px" }}>
            {/* Label */}
            <p style={{
              fontSize: 11, fontWeight: "bold", letterSpacing: "0.15em", textTransform: "uppercase",
              color: palette.q.label, marginBottom: 14
            }}>🌿 Question</p>
