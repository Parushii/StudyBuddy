...
async function uploadFile(filePath, fileName, parentFolderId) {
  ...
  const media = { mimeType: "application/pdf", body: fs.createReadStream(filePath) }; // Adjust mimeType if needed

  const uploadedFile = await drive.files.create({ resource: fileMetadata, media, fields: "id" });
